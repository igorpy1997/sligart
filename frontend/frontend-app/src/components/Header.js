...
const Header = () => {
    ...
    useEffect(() => {
        const loadDevelopers = async () => {
            setLoading(true);
            try {
                // Запрашиваем только 5 разработчиков, сортировка по order_priority встроена в API
                const response = await fetch('/api/public/developers?active_only=true&limit=5');
                if (response.ok) {
