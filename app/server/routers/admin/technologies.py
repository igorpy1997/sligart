...
async def get_technology(technology_id: int, db: AsyncSession = Depends(get_db_session)):
...
@router.post("")
async def create_technology(technology: TechnologyCreate, db: AsyncSession = Depends(get_db_session)):
...
async def delete_technology(technology_id: int, db: AsyncSession = Depends(get_db_session)):
    query = select(DBTechnologyModel).where(DBTechnologyModel.id == technology_id)
    result = await db.execute(query)
    db_technology = result.scalar_one_or_none()
...
async def get_technology_categories(db: AsyncSession = Depends(get_db_session)):
